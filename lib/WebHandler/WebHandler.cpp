...
void handleSetupPage(AsyncWebServerRequest *request)
{
  ...
  response->println("<form class=\"pure-form pure-form-aligned\" "
                    "action='/savecfg' method=POST><fieldset>");
  ...
  prCheckBoxGroup(response, id++, "Callback Enabled", "ohab_enabled",
                  appcfg.ohab_enabled);
  ...
  prTextGroup(response, id++, "Item Name", "ohab_itemname",
  ...
  prCheckBoxGroup(response, id++, "Use Authentication", "ohab_useauth",
                  appcfg.ohab_useauth);
  prTextGroup(response, id++, "User", "ohab_user", appcfg.ohab_user);
  prTextGroup(response, id++, "Password", "ohab_password",
              appcfg.ohab_password);

  // Alexa
  prLegend(response, "Alexa");
  prCheckBoxGroup(response, id++, "Enabled", "alexa_enabled",
                  appcfg.alexa_enabled);
  prTextGroup(response, id++, "Devicename", "alexa_devicename",
              appcfg.alexa_devicename);

  // MQTT
  prLegend(response, "MQTT");
  prCheckBoxGroup(response, id++, "Enabled", "mqtt_enabled",
                  appcfg.mqtt_enabled);
  prTextGroup(response, id++, "Client ID", "mqtt_clientid",
  ...
  prCheckBoxGroup(response, id++, "Use Authentication", "mqtt_useauth",
                  appcfg.mqtt_useauth);
  prTextGroup(response, id++, "User", "mqtt_user", appcfg.mqtt_user);
  prTextGroup(response, id++, "Password", "mqtt_password",
              appcfg.mqtt_password);
  prTextGroup(response, id++, "In Topic", "mqtt_intopic", appcfg.mqtt_intopic);
  prTextGroup(response, id++, "Out Topic", "mqtt_outtopic",
              appcfg.mqtt_outtopic);

  // Syslog
  prLegend(response, "Syslog");
  prCheckBoxGroup(response, id++, "Enabled", "syslog_enabled",
                  appcfg.syslog_enabled);
  ...
  prTextGroup(response, id++, "App Name", "syslog_app_name",
              appcfg.syslog_app_name);

  response->println("<p><input class='pure-button pure-button-primary' "
                    "type='submit' value='Save Configuration'></p>");
